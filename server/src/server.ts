...
connection.onCompletion((params: TextDocumentPositionParams): CompletionItem[] => {
	...
	if (line.type === LineType.Framebulk) {
		if (line.lineText.split('|').length - 1 !== 4) return [];
		...
		var tool = "";
		var index = params.position.character;
		while (index >= pipeIndex) {
			const char = line.lineText.charAt(index);
			if (char === ' ') {
				if (tool.length > 0) {
		...
		if (tool.length === 0) {
			...
			if (!TASTool.tools.hasOwnProperty(tool)) return [];
			if (encounteredWords.includes("off")) return [];

			const t = TASTool.tools[tool];
			const result: CompletionItem[] = [];
			if (encounteredWords.length === 0 && t.hasOff) {
				result.push({
					label: "off",
			...
			const toolArguments = t.arguments;
			result.push(...toolArguments.filter((arg) => {
